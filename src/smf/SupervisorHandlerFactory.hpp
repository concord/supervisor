...
class SupervisorHandlerFactory : public proxygen::RequestHandlerFactory {
 ...
 private:
  ...
  proxygen::RequestHandler *handleGetRequest(const boost::optional<int> &process_id);
  proxygen::RequestHandler *handleDeleteRequest(const boost::optional<int> &process_id);

  std::map<int, SubprocessPtr> processMap_;
};
}
